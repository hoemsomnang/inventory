  ...
  <div>
    ...
    <div class="form-grid">
      <el-card class="form-card">
        <el-form :model="form" label-width="140px" ref="formRef">
          ...
          <el-form-item label="Current Stock">
            <span class="stock-badge">{{ currentStock ?? '-' }} {{ selectedProduct?.unit || '' }}</span>
          </el-form-item>
          <el-form-item label="Quantity In" prop="quantity" required>
            <el-input-number v-model="form.quantity" :min="1" style="width:160px" />
          </el-form-item>
          <el-form-item label="Reference No.">
            <el-input v-model="form.referenceNo" placeholder="PO-2024-001" />
          </el-form-item>
          <el-form-item label="Notes">
            <el-input v-model="form.notes" type="textarea" :rows="3" />
          </el-form-item>
          <el-form-item>
            <el-button type="success" size="large" :loading="saving" @click="submit" style="width:100%">
              <el-icon><Download /></el-icon> &nbsp; Confirm Stock In
            </el-button>
          </el-form-item>
        </el-form>
      </el-card>
      ...
      <el-card class="recent-card">
        <template #header><span class="card-title">Recent Stock In</span></template>
        <div v-for="tx in recentIn" :key="tx.id" class="tx-item">
          <div>
            <div class="tx-product">{{ tx.product?.name }}</div>
            <div class="tx-ref">{{ tx.referenceNo || '—' }} · {{ formatDate(tx.createdAt) }}</div>
          </div>
          <el-tag type="success">+{{ tx.quantity }}</el-tag>
        </div>
        <div v-if="recentIn.length === 0" class="empty-tx">No records yet</div>
      </el-card>
    </div>
  </div>
</template>
...
async function submit() {
  ...
  saving.value = true
  try {
    await api.post('/stock/in', form.value)
    ElMessage.success('Stock In recorded successfully!')
    form.value = { productId: null, quantity: 1, referenceNo: '', notes: '' }
    currentStock.value = null
    selectedProduct.value = null
    await loadData()
  } catch (e) {
    ElMessage.error(e.response?.data?.error || 'Failed to record stock in')
...
async function loadData() {
  ...
  recentIn.value = tx.data.filter(t => t.type === 'IN').slice(0, 10)
}
...
.stock-badge { font-size: 20px; font-weight: 700; color: var(--success); }
